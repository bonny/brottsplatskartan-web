**Status:** klar 2026-05-01 — paketet installerat & konfigurerat redan 2026-04-23 (commit 9f9ef1b). Hybridlösning: egen `EventMarkdownRenderer` för event-sidor (kurerad metadata, 99 % mindre payload), Spatie konverterar resten (start, län, plats, statistik) automatiskt. Alla fyra detekterings-mekanismer aktiva: `.md`-suffix, `Accept: text/markdown`, AI-UA (ClaudeBot m.fl.), per-route via `ProvideMarkdown`/`DoNotProvideMarkdown`. Cache-separation via `useCacheNameSuffix('md')`. Todon skapades retroaktivt från inbox 2026-04-30 utan att inse att arbetet redan var gjort.
