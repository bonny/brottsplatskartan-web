...
Senast uppdaterad: 2026-05-15 (+#81 fas 1 implementerad lokalt — migration + LogAiUsage-listener på `AgentPrompted` + `ai:usage`-rapport + prune-job. PHPStan grön, smoke-test OK. Väntar push/deploy + prod-soak innan steg 0a/0b/0c).
...
| 81  | Håll koll på hur mycket AI-anropen kostar               | Fas 1 implementerad lokalt 2026-05-15 + PHPStan grön — smoke-test loggade NewsClassifier-anrop korrekt. Väntar push/deploy + prod-soak. Sen 0a (krympa prompts) + 0b (regex-pre-filter) + 0c (prompt-caching, kräver vendor-PR). | [todos/81-ai-kostnad-overvakning.md](todos/81-ai-kostnad-overvakning.md)                                             |
